...
for j in range(20000):
    ...
    # Change the sigmoid to your function
    ...
    # uncomment this line in order to use sigmoid function
    # a1_delta = a2_delta.dot(W2.T) * (a1 * (1 - a1))
    # print(a1)
    # uncomment this line in order to use tanh function
    a1_delta = a2_delta.dot(W2.T) * 4 / np.power((np.exp(a1) + np.exp(-a1)), 2)
    # uncomment this line in order to use Relu function
    #print(a1)
    #print(np.sign(a1))
    # a1_delta = a2_delta.dot(W2.T) * (a1>0)*1
    # UPDATE PARAMETERS
...
test_x1 = np.linspace(-1.5, 2.5, 40)
test_x2 = np.linspace(-1, 1.5, 40)
for x1 in test_x1:
    for x2 in test_x2:
        y = predict([[x1, x2]])
